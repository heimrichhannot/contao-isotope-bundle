 ...
 * Callbacks
 */
$arrDca['config']['ondelete_callback'][] = [
    'tl_iso_product_collection_isotope_plus',
    'increaseStock',
];
        ...
        if (($objOrder = \Isotope\Model\ProductCollection::findByPk($objDc->activeRecord->id)) !== null) {
            ...
            foreach ($objOrder->getItems() as $objItem) {
                if (($objProduct = $objItem->getProduct()) !== null) {
                    $intTotalQuantity = System::getContainer()->get('huh.isotope.manager')->getTotalStockQuantity($objItem->quantity, $objProduct, null, $objItem->setQuantity, $objConfig);
